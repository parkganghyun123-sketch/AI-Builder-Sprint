...
const STEPS = [
  ...
  { href: "/contract", label: "수정본" },
  { href: "/sign", label: "서명" },
  { href: "/archive", label: "보관" },
];
    ...
    <header className="sticky top-0 z-10 border-b border-brand-line/60 bg-white/80 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-3xl items-center justify-between px-5">
        <Link href="/" className="flex items-center gap-1.5 text-lg font-extrabold text-ink">
          <span className="text-brand">✓</span> 페어사인
        </Link>
        <Link href="/archive" className="text-sm font-semibold text-ink-muted hover:text-ink">
          보관함
        </Link>
      </div>
    </header>
                    ...
                    className={`rounded-full px-3 py-1 text-xs font-bold transition ${
                      active
                        ...
                        : done
                          ...
                          : "bg-white text-ink-soft border border-brand-line"
                    }`}
                  >
                    {n}. {s.label}
                  </li>
