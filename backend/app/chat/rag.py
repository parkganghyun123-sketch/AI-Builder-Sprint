...
def natural_generation_is_grounded(
    ...
    모델 출력에는 자유 텍스트 필드가 존재하지 않는다. 따라서 허용 ID를 붙인 비수치
    환각, 새 숫자, URL, 새 계산이나 확정적 판단은 Pydantic 검증 단계에서 스키마 밖
    필드로 거절되고 사용자에게 도달할 수 없다.
    """
...
async def _try_openai_generation(
    ...
    try:
        ...
        return None
    return _render_openai_explanation(generated, context)
...
async def enrich_with_grounded_rag(
    *,
    ...
    openai_explanation = await _try_openai_generation(response, context)
    if openai_explanation is not None:
        return response.model_copy(
            update={
                "answer": f"{response.answer}\n\n{openai_explanation}",
                "retrieved_knowledge": trace,
                "answer_mode": AnswerMode.OPENAI_GROUNDED_GENERATION,
            }
        )
